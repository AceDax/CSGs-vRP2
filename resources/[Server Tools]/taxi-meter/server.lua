  ...
  AddEventHandler('chatMessage', function(source, n, message)
    ...
    if (args[1] == "/create") then
      ...
      if (args[2] ~= nil) then
        ...
        local message = "Usage: /create <Vehicle_Name>  (for example /spawn polmav)"
...
AddEventHandler('chatMessage', function(from,name,message)
  if(string.sub(message,1,1) == "/") then
    ...
    if(cmd == "/taxifare")then
      ...
      if subCmd == "help" then
        TriggerClientEvent('chatMessage', from, "Taxi Meter -- Help", {200,0,0} , "Possible actions: initial, mile, minute, show. Initial is the base starting fare (default: $50). Mile is the rate per mile driven (default: $35). Minute is the rate per minute while the meter is on (default: $0). Show will display your current settings.")
        return
      end
      if subCmd == "show" then
