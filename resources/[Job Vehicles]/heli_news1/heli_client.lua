...
local helicam = false
local newsheli_hash = GetHashKey("newsheli")
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPlayerInnewsheli() and target_vehicle and not helicam and vehicle_display ~=2 then
...
function IsPlayerInnewsheli()
	local lPed = GetPlayerPed(-1)
	local vehicle = GetVehiclePedIsIn(lPed)
	return IsVehicleModel(vehicle, newsheli_hash)
end
