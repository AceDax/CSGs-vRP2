			...
			if IsPedInAnyVehicle(playerped, false) then	
				...
				if GetPedInVehicleSeat(veh, -1) == playerped then
					...
					if GetVehicleClass(veh) == 18 then
						...
						DisableControlAction(0, 85, true) -- INPUT_VEH_RADIO_WHEEL Q
						...
						if not IsPauseMenuActive() then
							...
							-- POWERCALL
							elseif IsDisabledControlJustReleased(0, 36) then
								if state_pwrcall[veh] == true then
									PlaySoundFrontend(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
									TogPowercallStateForVeh(veh, false)
									count_bcast_timer = delay_bcast_timer
					...
					if GetVehicleClass(veh) ~= 14 and GetVehicleClass(veh) ~= 15 and GetVehicleClass(veh) ~= 16 and GetVehicleClass(veh) ~= 21 then
						...
						if not IsPauseMenuActive() then
							...
							-- IND L
							if IsDisabledControlJustReleased(0, 174) then -- left arrow
							...
							elseif IsDisabledControlJustReleased(0, 175) then -- right arrow
								...
								count_bcast_timer = delay_bcast_timer
							-- IND H
							elseif IsControlJustReleased(0, 172) then -- up arrow
