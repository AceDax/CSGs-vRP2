...
local proned = false
crouchKey = 19 -- Left Alt
proneKey = 36 -- Left Control
modifierKey = 61 -- Left Shift
	...
	while true do 
		...
		if ( DoesEntityExist( ped ) and not IsEntityDead( ped ) ) then 
			...
			if ( not IsPauseMenuActive() ) then 
				if ( IsControlPressed( 0, modifierKey ) and IsControlPressed( 0, crouchKey ) and not proned and not IsPedInAnyVehicle(ped, true) ) then 
